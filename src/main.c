...
// Callback for selecting input file
static void on_select_input_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (res == GTK_RESPONSE_ACCEPT) {
        ...
        if (widgets->input_file_path) {
            g_free(widgets->input_file_path);
        }
        widgets->input_file_path = filename;
        
        // Display file info
        long size = get_file_size(filename);
        char info[512];
        snprintf(info, sizeof(info), "Input: %s (Size: %ld bytes)", filename, size);
        gtk_label_set_text(GTK_LABEL(widgets->input_file_label), info);
        gtk_label_set_text(GTK_LABEL(widgets->status_label), "Status: Input file selected");
    }
    
    gtk_widget_destroy(dialog);
}

// Callback for selecting output file
static void on_select_output_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (res == GTK_RESPONSE_ACCEPT) {
        ...
        if (widgets->output_file_path) {
            g_free(widgets->output_file_path);
        }
        widgets->output_file_path = filename;
        
        char info[512];
        snprintf(info, sizeof(info), "Output: %s", filename);
        gtk_label_set_text(GTK_LABEL(widgets->output_file_label), info);
        gtk_label_set_text(GTK_LABEL(widgets->status_label), "Status: Output file selected");
    }
...
static void on_compress_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (!widgets->input_file_path) {
        gtk_label_set_text(GTK_LABEL(widgets->status_label), "Status: Please select an input file");
        return;
    }
    
    if (!widgets->output_file_path) {
        gtk_label_set_text(GTK_LABEL(widgets->status_label), "Status: Please select an output file");
        return;
    }
    
    // Use file_manager compress function
    fm_status_t status = fm_compress(widgets->input_file_path, widgets->output_file_path);
...
static void on_decompress_clicked(GtkButton *button, gpointer user_data) {
    ...
    fm_status_t status = fm_decompress(widgets->input_file_path, widgets->output_file_path);
...
static void on_clear_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (widgets->input_file_path) {
        g_free(widgets->input_file_path);
        widgets->input_file_path = NULL;
    }
    
    if (widgets->output_file_path) {
        g_free(widgets->output_file_path);
        widgets->output_file_path = NULL;
    }
    
    gtk_label_set_text(GTK_LABEL(widgets->input_file_label), "Input: No file selected");
    gtk_label_set_text(GTK_LABEL(widgets->output_file_label), "Output: No file selected");
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    // Input file section
    GtkWidget *input_frame = gtk_frame_new("Input File");
    ...
    widgets->input_file_label = gtk_label_new("Input: No file selected");
    gtk_widget_set_halign(widgets->input_file_label, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(input_box), widgets->input_file_label, FALSE, FALSE, 5);
    
    GtkWidget *select_input_btn = gtk_button_new_with_label("Select Input File");
    ...
    widgets->output_file_label = gtk_label_new("Output: No file selected");
    gtk_widget_set_halign(widgets->output_file_label, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(output_box), widgets->output_file_label, FALSE, FALSE, 5);
    
    GtkWidget *select_output_btn = gtk_button_new_with_label("Select Output File");
    ...
    widgets->compress_btn = gtk_button_new_with_label("Compress File");
    ...
    widgets->decompress_btn = gtk_button_new_with_label("Decompress File");
